...
final class Template {
	...
	public static function get_part( $slug, $name = '' ) {
		...
		// Look in yourtheme/slug-name.php and yourtheme/marko-woocommerce-api-fetch/slug-name.php .
		if ( $name ) {
		...
		// If template file doesn't exist, look in yourtheme/slug.php and yourtheme/marko-woocommerce-api-fetch/slug.php .
		if ( ! $template ) {
			$template = locate_template( array( "{$slug}.php", Utils::template_path() . "{$slug}.php" ) );
		}

		// Allow 3rd party plugins to filter template file from their plugin.
		$template = apply_filters( 'marko_woocommerce_api_fetch_get_template_part', $template, $slug, $name );
	...
	public static function get( $template_name, $args = array(), $template_path = '', $default_path = '' ) {
		...
		$located = apply_filters( 'marko_woocommerce_api_fetch_get_template', $located, $template_name, $args, $template_path, $default_path );

		// Perform other actions before template part is included.
		do_action( 'marko_woocommerce_api_fetch_before_template_part', $template_name, $template_path, $located, $args );

		include $located;

		// Perform other actions after template part is included.
		do_action( 'marko_woocommerce_api_fetch_after_template_part', $template_name, $template_path, $located, $args );
	}
	...
	public static function locate( $template_name, $template_path = '', $default_path = '' ) {
		...
		return apply_filters( 'marko_woocommerce_api_fetch_locate_template', $template, $template_name, $template_path );
	}
}
